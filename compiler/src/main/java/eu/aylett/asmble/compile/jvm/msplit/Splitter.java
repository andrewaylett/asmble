...
public class Splitter implements Iterable<Splitter.SplitPoint> {
    ...
    protected int compareInsnIndexes(AbstractInsnNode o1, AbstractInsnNode o2) {
        return Integer.compare(
            method.instructions.indexOf(o1),
            method.instructions.indexOf(o2)
        );
    }
    ...
    public static class SplitPoint {
        /**
         * The locals read in this split area, keyed by index. Value type is always int, float, long,
         * double, or object.
         */
        public final SortedMap<Integer, Type> localsRead;
        /**
         * The locals written in this split area, keyed by index. Value type is always int, float, long,
         * double, or object.
         */
        public final SortedMap<Integer, Type> localsWritten;
        /**
         * The values of the stack needed at the start of this split area. Type is always int, float,
    ...
    protected static class StackAndLocalTrackingAdapter
    ...
    protected class Iter implements Iterator<SplitPoint> {
        ...
        protected SplitPoint longestForCurrIndex() {
            ...
            if (
                currIndex - 1 > -0 &&
                insns[currIndex - 1] instanceof LineNumberNode
            ) return null;
            ...
            // Now that we have our largest range from the start index, we can go over each updating the
            // local refs and stack
        ...
        protected void constrainEndByTryCatchBlocks(InsnTraverseInfo info) {
            ...
            for (TryCatchBlockNode block : tryCatchBlocks) {
                ...
                if (
                    info.startIndex <= startIndex && info.endIndex >= endIndex
                ) {
                    ...
                    if (
                        handleIndex < info.startIndex ||
                        handleIndex > info.endIndex
                    ) {
        ...
        protected void constrainEndByInternalJumps(InsnTraverseInfo info) {
            for (int i = info.startIndex; i <= info.endIndex; i++) {
                ...
                if (
                    earliestIndex < info.startIndex ||
                    furthestIndex > info.endIndex
                ) {
                    info.endIndex = i - 1;
                    return;
                }
        ...
        protected void constrainEndByExternalJumps(InsnTraverseInfo info) {
            ...
            for (int i = 0; i < insns.length; i++) {
                ...
                AbstractInsnNode node = insns[i];
                if (node instanceof JumpInsnNode) {
                    int index = method.instructions.indexOf(
                        ((JumpInsnNode) node).label
                    );
                    if (index >= info.startIndex) info.endIndex =
                        Math.min(info.endIndex, index - 1);
                } else if (node instanceof TableSwitchInsnNode) {
                    int index = method.instructions.indexOf(
                        ((TableSwitchInsnNode) node).dflt
                    );
        ...
        protected SplitPoint splitPointFromInfo(InsnTraverseInfo info) {
            // We're going to use the analyzer adapter and run it for the up until the end, a step at a
            // time
            StackAndLocalTrackingAdapter adapter = new StackAndLocalTrackingAdapter(
                Splitter.this
            );
            // Visit all of the insns up our start.
            // XXX: I checked the source of AnalyzerAdapter to confirm I don't need any of the surrounding
            // stuff
            ...
            return new SplitPoint(
                localMapFromAdapterLocalMap(
                    adapter.localsRead,
                    adapter.uninitializedTypes
                ),
                localMapFromAdapterLocalMap(
        ...
        protected SortedMap<Integer, Type> localMapFromAdapterLocalMap(
            ...
            Map<Object, Object> uninitializedTypes
        ) {
        ...
        protected List<Type> typesFromAdapterStackRange(
            ...
            for (int i = start; i < stack.size(); i++) {
                ...
                if (item == Opcodes.LONG || item == Opcodes.DOUBLE) {
                    if (
                        stack.get(++i) != Opcodes.TOP
                    ) throw new IllegalStateException(
                        "Expected top after long/double"
                    );
                }
            }
            return ret;
        ...
        protected Type typeFromAdapterStackItem(
            ...
            if (item == Opcodes.INTEGER) return Type.INT_TYPE; else if (
                item == Opcodes.FLOAT
            ) return Type.FLOAT_TYPE; else if (
                item == Opcodes.LONG
            ) return Type.LONG_TYPE; else if (
                item == Opcodes.DOUBLE
            ) return Type.DOUBLE_TYPE; else if (
                item == Opcodes.NULL
            ) return Util.OBJECT_TYPE; else if (
                item == Opcodes.UNINITIALIZED_THIS
            ) return Type.getObjectType(owner); else if (
                item instanceof Label
            ) return Type.getObjectType(
                (String) uninitializedTypes.get(item)
            ); else if (item instanceof String) return Type.getObjectType(
                (String) item
            ); else throw new IllegalStateException(
                "Unrecognized stack item: " + item
            );
        }
    }
}
