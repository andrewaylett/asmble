...
import static eu.aylett.asmble.compile.jvm.msplit.Util.*;
...
public class SplitMethod {
  ...
  protected MethodNode createSplitOffMethod(MethodNode orig, Splitter.SplitPoint splitPoint) {
    ...
    newMethod.visitTypeInsn(Opcodes.ANEWARRAY, Util.OBJECT_TYPE.getInternalName());
    ...
    for (Integer index : splitPoint.localsWritten.keySet()) {
      ...
      currIndex++;
      // Load the var
      newMethod.visitVarInsn(Util.loadOpFromType(item), localsMap.get(index));
      // Box it if necessary
      Util.boxStackIfNecessary(item, newMethod);
      // Store in array
      newMethod.visitInsn(Opcodes.AASTORE);
  ...
  protected MethodNode createTrimmedMethod(String owner, MethodNode orig,
    ...
    for (int i = 0; i < splitPoint.start; i++) {
      ...
      if (insn instanceof VarInsnNode && Util.isStoreOp(insn.getOpcode())) seenStoredLocals.add(((VarInsnNode) insn).var);
      insn.accept(newMethod);
    }
    ...
    splitPoint.localsRead.forEach((index, type) -> {
      ...
      if (seenStoredLocals.contains(index)) newMethod.visitVarInsn(Util.loadOpFromType(type), index);
    ...
    for (Integer index : splitPoint.localsWritten.keySet()) {
      ...
      if (!item.equals(Util.OBJECT_TYPE)) {
        newMethod.visitTypeInsn(Opcodes.CHECKCAST, Util.boxedTypeIfNecessary(item).getInternalName());
      }
      // Unbox if necessary
