package eu.aylett.asmble.compile.jvm.msplit;
...
public class SplitMethod {
    ...
    /**
     * @param api Same as for {@link org.objectweb.asm.MethodVisitor#MethodVisitor(int)} or any other
     *     ASM class
     */
     ...
     * original, maxSize as 70% + 1 of the original, and firstAtLeast as maxSize. The original method
     * is never modified and the result can be null if no split points are found.
     */
     ...
     * Splitter.SplitPoint}s until one reaches firstAtLeast or the largest otherwise, and then calls
     * {@link #fromSplitPoint(String, MethodNode, Splitter.SplitPoint)}.
     *
     * @param owner The internal name of the owning class. Needed when splitting to call the split off
     *     method.
    ...
    public Result split(
        ...
        for (Splitter.SplitPoint point : new Splitter(
            api,
            owner,
            method,
            minSize,
     ...
     */
    public Result fromSplitPoint(
        String owner,
        MethodNode orig,
        Splitter.SplitPoint splitPoint
    ) {
        MethodNode splitOff = createSplitOffMethod(orig, splitPoint);
        MethodNode trimmed = createTrimmedMethod(
            owner,
            orig,
            splitOff,
            splitPoint
    ...
    protected MethodNode createSplitOffMethod(
        MethodNode orig,
        Splitter.SplitPoint splitPoint
    ) {
        ...
        String name =
            orig.name.replace("<", "__").replace(">", "__") + "$split";
        ...
        for (int i = 0; i < splitPoint.neededFromStackAtStart.size(); i++) {
            ...
            newMethod.visitVarInsn(Util.loadOpFromType(item), i);
        }
        // Next set of instructions comes verbatim from the original, but we have to change the local
        // indexes
        ...
        for (int i = 0; i < splitPoint.length; i++) {
            ...
            // Store the label
            if (insn instanceof LabelNode) seenLabels.add(
                ((LabelNode) insn).getLabel()
            );
            // Change the local if needed
            if (insn instanceof VarInsnNode) {
                insn = insn.clone(Collections.emptyMap());
                ((VarInsnNode) insn).var =
                    localsMap.get(((VarInsnNode) insn).var);
        ...
        int retArrSize =
            splitPoint.putOnStackAtEnd.size() + splitPoint.localsWritten.size();
        Util.intConst(retArrSize).accept(newMethod);
        newMethod.visitTypeInsn(
            Opcodes.ANEWARRAY,
            Util.OBJECT_TYPE.getInternalName()
        );
        ...
        // Now go over each stack item and load the arr, swap w/ the stack, add the index, swap with the
        // stack, and store
        ...
        for (Integer index : splitPoint.localsWritten.keySet()) {
            ...
            newMethod.visitVarInsn(Opcodes.ALOAD, retArrLocalIndex);
            // Add the arr index
            Util.intConst(currIndex).accept(newMethod);
            currIndex++;
            // Load the var
            newMethod.visitVarInsn(
                Util.loadOpFromType(item),
                localsMap.get(index)
            );
            // Box it if necessary
            Util.boxStackIfNecessary(item, newMethod);
        ...
        for (TryCatchBlockNode tryCatch : orig.tryCatchBlocks) {
            if (seenLabels.contains(tryCatch.start.getLabel())) tryCatch.accept(
                newMethod
            );
        }
        // Reset the labels
    ...
    protected MethodNode createTrimmedMethod(
        ...
        // The trimmed method is the same as the original, yet the split area is replaced with a call to
        // the split off
        ...
        for (int i = 0; i < paramCount; i++) seenStoredLocals.add(
            i + paramOffset
        );
        ...
        for (int i = 0; i < splitPoint.start; i++) {
            ...
            if (
                insn instanceof VarInsnNode && Util.isStoreOp(insn.getOpcode())
        ...
        splitPoint.localsRead.forEach(
            (index, type) -> {
                ...
                // TODO: safe? if not, maybe just put at the top of the method a bunch of defaulted
                // locals?
                if (seenStoredLocals.contains(index)) newMethod.visitVarInsn(
                    Util.loadOpFromType(type),
                    index
                ); else Util.zeroVal(type).accept(newMethod);
            }
        );
        ...
        for (Integer index : splitPoint.localsWritten.keySet()) {
            ...
            Util.intConst(localArrIndex).accept(newMethod);
            localArrIndex++;
            // Load the written local
            Type item = splitPoint.localsWritten.get(index);
            newMethod.visitInsn(Opcodes.AALOAD);
            // Cast to local type
            if (!item.equals(Util.OBJECT_TYPE)) {
                newMethod.visitTypeInsn(
                    Opcodes.CHECKCAST,
                    Util.boxedTypeIfNecessary(item).getInternalName()
                );
            }
            // Unbox if necessary
            Util.unboxStackIfNecessary(item, newMethod);
            // Store in the local
            newMethod.visitVarInsn(Util.storeOpFromType(item), index);
        }
        ...
        for (int i = 0; i < splitPoint.putOnStackAtEnd.size(); i++) {
            ...
            if (!last) {
                // Note if the stack item takes two slots, we do a form of dup then pop since there's no
                // swap1x2
        ...
        for (
            int i = splitPoint.start + splitPoint.length;
            i < orig.instructions.size();
            i++
        ) {
            AbstractInsnNode insn = orig.instructions.get(i);
            // Skip frames
