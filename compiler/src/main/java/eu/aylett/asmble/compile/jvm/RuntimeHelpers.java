...
class RuntimeHelpers {
    ...
    static CallSite bootstrapIndirect(
        MethodHandles.Lookup caller,
        String name,
        MethodType type
    )
        throws Exception {
        MethodType withoutIndexOrThis = type.dropParameterTypes(
        ...
        for (int i = 0; i < reorder.length; i++) {
            if (i == 0) reorder[0] = reorder.length - 1; else reorder[i] =
                i - 1;
        }
        // Method handle that moves the method handle to the end
        MethodHandle mhAtEnd = MethodHandles.permuteArguments(
            mhFirst,
            mhFirst
                .type()
                .dropParameterTypes(0, 1)
                .appendParameterTypes(MethodHandle.class),
            reorder
        );
        // Method handle that changes an ending table + index to a method handle using the table
        MethodHandle tableAndIndexAtEnd = MethodHandles.collectArguments(
            mhAtEnd,
            mhAtEnd.type().parameterCount() - 1,
            MethodHandles.arrayElementGetter(MethodHandle[].class)
        );
        // Method handle that changes second-to-last this to table
        MethodHandle thisAndIndexAtEnd = MethodHandles.filterArguments(
            tableAndIndexAtEnd,
