...
fun <T : Exception> KClass<T>.athrow(msg: String) = listOf(
    ...
    msg.const,
    MethodInsnNode(
        Opcodes.INVOKESPECIAL, this.ref.asmName, "<init>",
...
fun KClass<*>.invokeStatic(name: String, retType: KClass<*>, vararg params: KClass<*>) =
    MethodInsnNode(
        Opcodes.INVOKESTATIC, this.javaObjectType.ref.asmName, name,
...
val Class<*>.valueType: Node.Type.Value?
    get() = when (this) {
        Void.TYPE -> null
...
val Int.const: AbstractInsnNode
    get() = when (this) {
...
val Long.const: AbstractInsnNode
    get() = when (this) {
...
val Float.const: AbstractInsnNode
    get() = when (this) {
...
val Double.const: AbstractInsnNode
    get() = when (this) {
        // Ref: https://discuss.kotlinlang.org/t/when-isnt-equals/2452
...
val String.javaIdent: String
    get() {
        ...
        return if (this.isEmpty()) "\$empty\$"
            ...
            (if (this.first().isJavaIdentifierStart()) this else "wasm\$$this").fold(StringBuilder()) { builder, char ->
...
fun Node.Func.localByIndex(index: Int) =
    this.type.params.getOrNull(index) ?: this.locals.getOrNull(index - this.type.params.size)
...
val Node.Type.Value.kclass: KClass<*>
    get() = when (this) {
...
val AbstractInsnNode.isUnconditionalJump: Boolean
    get() = when (this.opcode) {
...
val Node.Type.Func.asmDesc: String
    get() =
