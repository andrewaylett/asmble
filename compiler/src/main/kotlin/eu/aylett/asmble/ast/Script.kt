...
data class Script(val commands: List<Cmd>) {
    sealed class Cmd {
        ...
        sealed class Meta : Cmd() {
            data class Script(val name: String?, val script: eu.aylett.asmble.ast.Script) : Meta()
    ...
    sealed class LazyModule(l: Lazy<Node.Module>) : Lazy<Node.Module> by l {
        data class SExpr(
            val sexpr: eu.aylett.asmble.ast.SExpr.Multi,
            val fn: (eu.aylett.asmble.ast.SExpr.Multi) -> Node.Module
